...
public class NPCHandler {
    ...
    public static void handleOption222(Player player, InputStream stream) {
        ...
        if (plugin != null) {
            player.setRouteEvent(new RouteEvent(npc, () -> {
                ...
                if (!plugin.processNpc2(player, npc)) {
                    Logger.log("NpcPlugin",
                            "Option2 missing: " + npc.getName() + "(" + npc.getId() + ")");
                }
            }, true));
            return;
        }

        player.setRouteEvent(new RouteEvent(npc, () -> handleNpcOption2(player, npc), true));
    }

    public static void handleNpcOption2(Player player, NPC npc) {
    ...
    public static void handleOption2(final Player player, InputStream stream) {
        ...
        player.setRouteEvent(new RouteEvent(npc, () -> {
            ...
            if (npc.getDefinitions().name.toLowerCase().equals("void knight")) {
